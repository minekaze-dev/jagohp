...
const CatalogCard: React.FC<{ item: CatalogItem; onOpenDetail: (item: CatalogItem) => void }> = ({ item, onOpenDetail }) => (
  <div className="bg-[#0c0c0c] border border-white/5 p-5 rounded-3xl flex flex-col hover:border-yellow-400/20 transition-all group shadow-xl relative overflow-hidden h-full">
    <div className="absolute top-0 right-0 w-24 h-24 bg-yellow-400/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl"></div>
    
    {/* Content Wrapper dengan flex-1 untuk mendorong tombol ke bawah */}
    <div className="flex-1 flex flex-col gap-4 relative z-10">
      <div className="flex justify-between items-start">
        <div className="space-y-1">
          ...
          <h3 className="text-sm md:text-base font-black text-white uppercase italic tracking-tighter group-hover:text-yellow-400 transition-colors leading-tight">{item.name}</h3>
        </div>
      ...
      <div className="space-y-2 mt-2">
        <div className="flex items-center gap-2 text-gray-500 text-[9px] md:text-[10px] font-medium italic">
        ...
        <div className="text-white font-black text-xs md:text-sm tracking-tight">{item.price}</div>
      </div>
    </div>

    {/* Tombol Kuning yang selalu di bawah */}
    <button 
...
const DetailModal: React.FC<{ item: CatalogItem; onClose: () => void }> = ({ item, onClose }) => (
  <div className="fixed inset-0 z-[1000] flex items-center justify-center p-2 md:p-4">
    ...
    <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-6xl max-h-[92vh] rounded-[2rem] md:rounded-[3rem] p-5 md:p-8 relative overflow-y-auto custom-scrollbar shadow-[0_0_100px_rgba(0,0,0,0.5)] animate-in zoom-in duration-300">
      ...
      {/* Header Modal - Diperketat untuk mobile */}
      <div className="flex justify-between items-start mb-6 border-b border-white/5 pb-4 sticky top-0 bg-[#0a0a0a] z-20">
        <div className="flex items-start gap-3 md:gap-5">
          ...
          <div className="space-y-1">
            ...
            {/* Nama HP: Menghilangkan truncate, mengizinkan wrap hingga 2 baris */}
...
const Catalog: React.FC = () => {
  ...
  const filters = {
    Brand: [
      ...
      'Infinix', 'Oppo', 'Vivo', 'Realme', 'Itel', 
      'Tecno', 'Honor', 'Huawei', 'Motorola', 'Red Magic'
    ],
    Harga: ['1-2 Juta', '2-3 Juta', '3-5 Juta', '5-10 Juta', 'Diatas 10 Juta'],
    Kebutuhan: ['Gaming', 'Harian', 'Fotografi', 'Konten Kreator', 'Baterai Awet']
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setAllItems(data);
        // Set default filter based on first tab (Brand)
  ...
  const filteredItems = useMemo(() => {
    ...
    // Pengurutan berdasarkan rilis terbaru
    ...
    <div className="max-w-[1000px] mx-auto px-4 py-16 space-y-12 pb-32">
      <div className="text-center space-y-4">
        <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter italic text-white leading-none">
          <span className="text-yellow-400">Kata</span>log
        </h1>
          ...
          {activeTab === 'Brand' ? (
            <div className="flex flex-col items-center gap-2 w-full animate-in fade-in duration-500">
              <div className="flex flex-wrap justify-center gap-2">
                {filters.Brand.slice(0, 8).map(f => renderFilterButton(f))}
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {filters.Brand.slice(8).map(f => renderFilterButton(f))}
              </div>
            </div>
          ) : (
      ...
      {loading ? (
        <div className="py-20 text-center animate-pulse space-y-6">
          ...
          <p className="text-gray-600 font-black uppercase tracking-[0.4em] text-[10px]">Memindai Database Smart Review...</p>
        </div>
      ) : (
