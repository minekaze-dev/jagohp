...
const NewsCard: React.FC<{ item: BlogPostExtended }> = ({ item }) => {
  ...
  return (
    <article className="group flex flex-col md:flex-row bg-white dark:bg-[#0a0a0a] border border-black/5 dark:border-white/5 rounded-[1.5rem] overflow-hidden transition-all duration-300 hover:border-yellow-400/20 shadow-xl theme-transition">
      ...
      <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <span className="text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded bg-yellow-400 text-black inline-block">
              {item.category}
            </span>
          ...
          <Link to={`/blog/${item.slug}`}>
            <h4 className="text-sm md:text-base font-black text-black dark:text-white uppercase italic tracking-tighter group-hover:text-yellow-400 transition-colors line-clamp-2 leading-tight">
              {item.title}
            </h4>
          </Link>
          <p className="text-gray-500 dark:text-gray-600 text-[11px] leading-relaxed italic line-clamp-1">{item.excerpt}</p>
        </div>
        ...
        <div className="pt-4 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={shareMenuRef}>
              <button 
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-xl border transition-all active:scale-90 ${showShare ? 'bg-yellow-400 border-yellow-400 text-black' : 'bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/10 text-gray-400 hover:text-black dark:hover:text-white hover:border-black/20 dark:hover:border-white/20'}`}
              >
              ...
              {showShare && (
                <div className="absolute bottom-full mb-2 left-0 bg-white dark:bg-neutral-900/95 backdrop-blur-xl border border-black/5 dark:border-white/10 p-1.5 rounded-xl flex gap-1.5 shadow-2xl animate-in fade-in slide-in-from-bottom-2 duration-200 z-50">
                  <a 
                  ...
                  <button 
                    onClick={handleCopy}
                    className={`w-8 h-8 flex items-center justify-center border rounded-lg transition-all ${copied ? 'bg-yellow-400 border-yellow-400 text-black' : 'bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/10 text-gray-400 hover:text-black dark:hover:text-white'}`}
                  >
                    {copied ? (
          ...
          <div className="flex gap-4 text-gray-400 dark:text-gray-700 text-[8px] md:text-[9px] font-black uppercase tracking-widest border-t xs:border-t-0 border-black/5 dark:border-white/5 pt-3 xs:pt-0 w-full xs:w-auto">
...
const Home: React.FC = () => {
  ...
  const features = [
    ...
    {
      title: 'Phone Match',
      description: 'Cari HP sesuai kebutuhan.',
      path: '/match',
  ...
  return (
    <div className="max-w-[900px] mx-auto px-4 pt-4 space-y-10 pb-20 theme-transition">
      <section className="text-center space-y-4 pt-4">
        <h1 className="text-2xl md:text-4xl font-black tracking-tighter uppercase italic text-black dark:text-white">
          Platform Rekomendasi<span className="text-yellow-400"> HP Berbasis AI</span>
        </h1>
        <p className="text-gray-500 dark:text-gray-400 text-lg md:text-base max-w-2xl mx-auto leading-relaxed italic">
      ...
      <section className="grid md:grid-cols-3 gap-6">
        {features.map((feature, idx) => (
          <Link 
            key={idx} 
            to={feature.path}
            className="group relative overflow-hidden rounded-3xl bg-white dark:bg-[#0a0a0a] border border-black/5 dark:border-white/5 hover:border-yellow-400/30 transition-all duration-300 flex flex-col shadow-2xl theme-transition"
          >
            ...
            <div className="p-6 space-y-3">
              <h3 className="text-xl font-black uppercase italic group-hover:text-yellow-400 transition-colors text-black dark:text-white">{feature.title}</h3>
              <p className="text-gray-500 dark:text-gray-500 text-sm leading-relaxed italic">"{feature.description}"</p>
            </div>
          </Link>
        ))}
      </section>
      ...
      {newsItems.length > 0 && (
        <section className="space-y-8 pt-0">
          <div className="flex items-end justify-between border-b border-black/5 dark:border-white/5 pb-2">
            <h2 className="text-2xl font-black uppercase italic tracking-tighter text-black dark:text-white">Berita <span className="text-yellow-400">Terbaru</span></h2>
            <Link to="/blog" className="text-gray-400 dark:text-gray-500 text-[10px] font-black uppercase hover:text-black dark:hover:text-white border-b border-gray-200 dark:border-gray-800 pb-1">Lihat Semua</Link>
          </div>
