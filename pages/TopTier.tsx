...
const TierCard: React.FC<{ phone: TopTierPhone }> = ({ phone }) => (
  <div className="bg-[#0c0c0c] border border-white/5 rounded-[2.5rem] p-8 md:p-10 space-y-6 shadow-2xl relative overflow-hidden group hover:border-yellow-400/30 transition-all duration-500 w-full animate-in fade-in zoom-in duration-700">
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
      <div className="flex items-center gap-6">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-black italic shadow-2xl transform transition-transform group-hover:scale-110 duration-500 bg-yellow-400 text-black shadow-yellow-400/20">
          #1
        </div>
        <div>
      ...
      <div className="bg-yellow-400/10 border border-yellow-400/20 text-yellow-400 px-5 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.3em] animate-pulse">
        🏆 Pilihan Terbaik
      </div>
    </div>

    <div className="relative">
      <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gradient-to-b from-yellow-400/50 to-transparent rounded-full"></div>
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-16 space-y-12 pb-32">
      <div className="text-center space-y-4">
        <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter italic"><span className="text-yellow-400">Top</span> Tier Rank</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-12 animate-in fade-in duration-700">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-8 md:p-12 text-center space-y-4 shadow-inner relative overflow-hidden">
            <div className="absolute inset-0 bg-yellow-400/[0.02] pointer-events-none"></div>
            <h2 className="text-2xl md:text-3xl font-black uppercase italic tracking-tighter relative z-10">Best of <span className="text-yellow-400">{data.category}</span></h2>
            <p className="text-gray-500 text-xs md:text-base italic font-medium max-w-xl mx-auto relative z-10">"{data.description}"</p>
          </div>
