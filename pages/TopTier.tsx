...
const TierCard: React.FC<{ phone: TopTierPhone }> = ({ phone }) => (
  <div className="bg-[#0c0c0c] border border-white/5 rounded-[2.5rem] p-8 md:p-12 space-y-8 shadow-2xl relative overflow-hidden group hover:border-yellow-400/30 transition-all duration-500 w-full">
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
      <div className="flex items-center gap-6">
        <div className={`w-16 h-16 rounded-3xl flex items-center justify-center text-3xl font-black italic shadow-2xl transform transition-transform group-hover:scale-110 duration-500 ${
          phone.rank === 1 ? 'bg-yellow-400 text-black shadow-yellow-400/20' : 'bg-white/5 text-white border border-white/10'
        }`}>
          #{phone.rank}
        </div>
        <div>
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-16 space-y-12 pb-32">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-500 text-sm md:text-base font-medium italic max-w-xl mx-auto">3 Top Tier Smartphone per Januari 2026 menurut Jagohp.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-16 animate-in fade-in duration-700">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-10 md:p-14 text-center space-y-6 shadow-inner relative overflow-hidden">
            ...
            <h2 className="text-2xl md:text-4xl font-black uppercase italic tracking-tighter relative z-10">The Best of <span className="text-yellow-400">{data.category}</span></h2>
          ...
          <div className="flex flex-col gap-8">
            {data.phones && data.phones.length > 0 ? (
              [...data.phones]
                ...
                .map((phone, i) => (<TierCard key={i} phone={phone} />))
            ) : (
              <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]"><p className="text-gray-700 font-black uppercase text-xs italic">Data belum diperbarui bulan ini.</p></div>
            )}
          </div>
